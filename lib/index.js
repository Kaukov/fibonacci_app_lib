...
const fibonacciTable = (length) => {
    ...
    // If we have a non-integer value, print an error message
    if (isNaN(intLength)) {
    ...
    if (intLength < 1) {
        return getBadValueErr(intLength)
    }

    // If a value higher than the max integer value is provided,
    // return an error message string
    if (intLength > Number.MAX_SAFE_INTEGER) {
        return 'Exceeding the max allowed integer value'
    }

    return Object.values(fibonacci(intLength, true))
