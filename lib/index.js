/**
 * printBadValueErr
 * Prints a message to the console when a value with no Fibonacci multiplication
 * table is provided, or a negative value is provided
 *
 * @param {number|string} value
 */
const printBadValueErr = (value) => {
	const intVal = parseInt(value)

	if (intVal < 0) {
		return console.log('Please provide a positive integer value!')
	}
	...
	console.log(`Nothing to display for the ${value}${ordinalStr} number.`)
}
...
const fibonacciTable = (length) => {
	...
	if (length < 4) {
		return printBadValueErr(length)
	}

	//
	if (length > Number.MAX_SAFE_INTEGER) {
		return console.log('Exceeding the max allowed integer value')
	}
